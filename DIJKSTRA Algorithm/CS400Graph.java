...
public class CS400Graph<NodeType, EdgeType extends Number> implements GraphADT<NodeType, EdgeType> {
	...
	/**
	 * This method uses Dijkstra's shortest path algorithm to find and return the
	 * shortest path between two vertices in this graph: start and end. This path
	 * contains an ordered list of the data within each node on this path, and also
	 * the distance or cost of all edges that are a part of this path.
	 * 
	...
	protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
		...
		if (!this.containsVertex(start) || !this.containsVertex(end)) {
			...
			throw new NoSuchElementException("The vertex does NOT exist!!!");
		}

		PriorityQueue<Path> dijkstrasPath = new PriorityQueue<Path>();

		Hashtable<NodeType, Path> shortestPath = new Hashtable<>();
		...
		shortestPath.put(start, newp);
		dijkstrasPath.add(newp);

		while (!dijkstrasPath.isEmpty()) {
			newp = dijkstrasPath.remove();
	...
	public List<NodeType> shortestPath(NodeType start, NodeType end) {
		...
		Path path = dijkstrasShortestPath(start, end);
		if (!this.containsVertex(start) || !this.containsVertex(end)) {
